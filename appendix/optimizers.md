# 附录 B · 优化器速查：从梯度下降到 AdamW

训练时显存被谁占掉了？很大一块是优化器的状态。这一页讲清楚 Adam 每个参数额外存了什么、为什么非存不可。

从最朴素的梯度下降开始，每加一个部件都先说它解决什么问题，加完再看它多占多少显存。最后用一个能在纸上算的例子，把 Adam 走两步。
...
$m$、$v$ 必须是 fp32：它们每步只变一点点（$v$ 每步加 $0.001\,g^2$），而 bf16 只有两三位有效数字，比当前值的 $0.8\%$ 还小的增量会被舍入掉，$m$、$v$ 就卡住不动了（为什么是 $0.8\%$，见[附录 C](numeric-formats.md)）。这就是 Day 00 §2.4 那张表里 Adam $m$、Adam $v$ 各占 4 字节的来源；加上 fp32 正本 4 字节和 bf16 的权重、梯度各 2 字节，每个可训练参数 16 字节。**冻结的参数一个 $m$、$v$ 都不需要**——LoRA 省显存的账就是这么算的。
