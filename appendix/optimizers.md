...
看懂 [Day 00 §2.4](../days/day00_lora-quickstart/) 的显存账需要知道 Adam 每个参数存了什么、为什么要存。这一页从最朴素的梯度下降开始，每加一个部件都先说它解决什么问题，最后用一个能手算的例子把 Adam 走两步。
...
$m$、$v$ 必须是 fp32：$v$ 是梯度平方的平均，bf16 的动态范围存不住 $10^{-10}$ 量级的数，会直接下溢成 0，然后 $\sqrt{v}$ 除零。这就是 Day 00 §2.4 那张表里 Adam $m$、Adam $v$ 各占 4 字节的来源；加上 fp32 主副本 4 字节和 bf16 的权重、梯度各 2 字节，每个可训练参数 16 字节。**冻结的参数一个 $m$、$v$ 都不需要**——LoRA 省显存的账就是这么算的。
