...
微调是把通用模型改成自己需要的样子最直接的手段。读文章能知道 LoRA 是什么，但只有真训一次才知道：显存会涨到多少、loss 降到多少算正常、学习率（learning rate）高一点会发生什么、多少条数据才够。这些数字不自己撞一遍是记不住的。
...
也就是说：**秩高于 $r$ 的那些 $\Delta W$，LoRA 根本表示不出来，训练再久也到不了。** 这和“近似”不是一回事——不是先算出真正的 $\Delta W$ 再拿低秩（low-rank）去逼近它，而是从头到尾只更新 $A$ 和 $B$，那些高秩的解从来没被碰过。
...
而且由 §2.3，可训练参数只有约 $2\times10^7$ 个。**参数少，需要的数据也少**——几百条不至于欠拟合（underfitting），还正好能让你亲手把每一条看一遍。
...
前面的 $d_{\text{in}}$、$d_{\text{out}}$ 一直是抽象符号。这一节把它们落到这个模型上，**只为回答两个问题**：LoRA 挂在哪几个矩阵上，以及为什么加起来正好是 43.3 M 个参数。

> [!TIP]
> **没学过 Transformer 也能过这一节**
>
> 下面会出现注意力、GQA、RoPE、GatedDeltaNet 这些名字。**今天一个都不需要懂**——它们是 Phase 1（day 02–04）的内容。想现在就搞清楚 Q、K、V 到底在干什么，看 [附录 D](../../appendix/transformer.md)，那里有一个三个 token、能拿笔算完的例子。今天只需要接受一件事：
>
> **这个模型是一叠 32 个结构相同的“块”，每个块里装着十来个矩阵。LoRA 做的事就是给其中每个矩阵挂一对小矩阵 $A$、$B$。**
>
> 所以这一节你真正要看的是：一个块里有**几个**矩阵、各自多大。矩阵叫什么名字、它在注意力里扮演什么角色，今天可以跳过。

一个块干两件事：先让不同位置的 token 互相看一眼（这部分叫 mixer），再对每个位置各自做一次变换（这部分叫前馈网络，feed-forward network，简称 FFN）。两件事都是靠矩阵乘法做的，而**矩阵乘法就是 `nn.Linear`**——LoRA 能挂的地方，就是这些 `nn.Linear`。

下面每个数字都来自模型自己的 `config.json`，用仓库里的工具几秒钟就能复现——它在 `meta` 设备上把模型搭出来，只建模块不读权重，所以不占显存也不用等加载：
...
两种块的区别今天只影响一件事：**它们的矩阵个数和名字不一样**（8 个 vs 7 个），所以数参数时要分开数。至于它们各自怎么算，day 03 讲 KV cache 时才需要：
...
> [!NOTE]
> **图里那些名词各是什么（今天不需要，好奇再看）**
>
> - **残差连接（residual connection）**：把一个模块的输出加回它自己的输入（图里的 ⊕）。这样梯度能顺着加法直接回到浅层，几十层的网络才训得动。
