...
全参微调直接学 $\Delta W$，那是每个矩阵 1678 万个自由参数。LoRA 的做法是把 $\Delta W$ 限制成 $BA$ 的形式。要看懂这一步，需要三件事：**秩是什么**（定义）、**为什么秩 $\le r$ 的矩阵一定能写成 $BA$**（定理，可证）、以及**凭什么认为 $\Delta W$ 的秩很低**（经验发现，不是定理）。这三件事的性质不一样，下面分开说。
