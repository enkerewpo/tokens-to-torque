...
`code/chat.html` 是一个自包含的客户端：没有构建、没有依赖，一个文件。它做的事就是 §2.2 说的那句话——接口长得一样，客户端直接指过来。

```bash
cd code && python3 -m http.server 8181       # 在板子上起个静态服务
# 浏览器打开 http://<板子地址>:8181/chat.html
```
...
> **从别的机器打开时的两个坑**
>
> 浏览器直接双击本地文件（`file://`）或从另一台机器访问时，请求要跨域。vLLM 默认 `allow_origins=["*"]`，所以服务端这边是放行的；但 Chrome 对"本机页面访问私有网段"另有限制，最省事的做法就是**把页面托管在跑模型的那台机器上**，像上面那样。
...
5. **`--gpu-memory-utilization` 在统一内存上切的是整块内存**（§2.4），不是独显那种"反正显存是我的"。这台机器上还跑着别的容器，切太狠会把系统挤爆。
