...
我做具身智能相关的系统工作。做着做着发现自己对脚下这一整层——模型怎么被服务、kernel 怎么跑、模型怎么训出来、VLM/VLA/WAM 到底是什么——只有二手知识。

所以有了这个仓库：**72 天，每天 2 小时，把这一整个栈从下到上亲手过一遍**。教程里的实测数字跑在一块 Jetson AGX Thor 上，但内容不绑定它——见下面的硬件分档。
